...
// V2: Use timing-based approach with proper beat calculation
export function NotationViewV2({ tune, transpose, progress, isPlaying, highlightOffset = 0 }: NotationViewProps) {
  ...
  // Update highlighting based on progress using beat timing
  useEffect(() => {
    if (!tune || noteElementsRef.current.length === 0) return;

    // Calculate total duration in beats (each section plays twice)
    let totalBeats = 0;
    ...
    const currentBeat = progress * totalBeats - highlightOffset;
    const adjustedBeat = Math.max(0, Math.min(totalBeats, currentBeat));

    // Find which note is playing at this beat
    let noteOffset = 0;
    let beatOffset = 0;
    let foundNoteIdx = -1;
    ...
    for (const section of tune.sections) {
      ...
      // Check both passes of this section
      for (let pass = 0; pass < 2; pass++) {
        if (adjustedBeat >= beatOffset && adjustedBeat < beatOffset + sectionBeats) {
          // We're in this section/pass
          const beatInSection = adjustedBeat - beatOffset;
          ...
          for (let i = 0; i < section.notes.length; i++) {
            ...
            if (beatInSection >= note.start_time && beatInSection < note.start_time + note.duration) {
              foundNoteIdx = noteOffset + i;
              break;
            }
          }

          // If between notes, use the last note that started
          if (foundNoteIdx === -1) {
      ...
      if (foundNoteIdx !== -1) break;
      noteOffset += section.notes.length;
    }

    // Update highlighting
    if (foundNoteIdx !== lastHighlightedRef.current) {
      ...
      // Add new highlight
      if (foundNoteIdx >= 0 && foundNoteIdx < noteElementsRef.current.length) {
        noteElementsRef.current[foundNoteIdx].classList.add('abcjs-note-playing');
      }
      lastHighlightedRef.current = foundNoteIdx;
    }
  ...
  return (
    <div className="notation-view notation-view-v2">
      <h2>{tune.title}</h2>
